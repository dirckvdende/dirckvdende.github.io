...
In our problem we have an image, which is partially transparent. Let's use the following sprite as an example:
...
One solution is to go over every pixel. Then, if this pixel is on the edge of the sprite, draw a black circle around it. When you only color those pixels black that were transparent before, you get a perfect outline effect.
...
However, notice how I mentioned "distance"? This isn't the usual "Euclidean" distance, this is the "Manhattan" distance, where the distance is measured as the sum of the distances along the x and y coordinates. This causes some funky results, where the outline appears much thinner along diagonal parts of the image:
...
Instead of repeatedly painting the *four* direct neighbours, we could also paint all *eight* neighbours, including diagonals. This has the exact opposite effect, with the outline being thicker along diagonal parts of the image:
...
Of course there are still shortcomings to an algorithm like this. For one, you can still see that it is drawing an octagon when you create a very thick outline on a very small sprite. Another is that it is not parallelizable, unlike the "slow solution" I described (which can make it faster in reality, at least on a GPU). However, I still thought this little algorithm was worth sharing!
